...
namespace Project1
{
    public class Friend3 : Friend
    {
        Enemy target;
        int cnt , rotateCnt;
        public Friend3(Texture2D texture, Vector2 pos, int width, int height, float ofUp, float ofDw, float ofLf,
            float ofRt, int hl) : base(texture , pos , width , height , ofUp , ofDw , ofLf , ofRt , hl)
        {
            frame = rotateCnt = 0;
            cnt = 300;
            target = null;
        ...
        public override void Update(GameTime gameTime, List<Sprite> sprites, List<Bullet> bullets, ref int idx)
        {
            ...
            if (r % 120 == 0)
            {
                GenerateBullet("8", "lunch_rocket", bullets, 100, 30, 1, 0);
            }

            if (idle == true)
            {
                cnt = 300;
                Enemy enm = GetRandEnemy(sprites);
                if (enm != null)
                {
                    target = enm;
            ...
            {
                if (sprites[i] is Enemy && Collision(this, sprites[i]))
                {
                    Globals.enemyCnt--;
